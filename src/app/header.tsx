...
/**
 * The header has a layout of the JWT Pizza Logo and name on the left, a hidden button?, then all the navigation links that
 *   belong in the header, ending with an account circle and letters if the user is logged in.
 * @param props (user, navItems)
 */
export default function Header(props: Props) {
  /**
   * Calls the navItem's constraint (such as isLoggedIn) to see if this navigation link matches the constraint and should
   *   be visible to the user
   * @param constraints (navItems.constraints)
   * @returns boolean
   */
  ...
  /**
   * Grabs the user's name, and splits the string name into an array when it sees a space. If the user only has one name,
   *   then returns the first letter of the first name. If the user has more than one name, then returns the first letter
   *   of the first name, followed by the first letter of the last name.
   * @param user
   * @returns a 1-2 letter nickname for the user's account circle
   */
